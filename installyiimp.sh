...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    debconf-set-selections <<< "mariadb-server mysql-server/root_password password $rootpasswd"
    debconf-set-selections <<< "mariadb-server mysql-server/root_password_again password $rootpasswd"
